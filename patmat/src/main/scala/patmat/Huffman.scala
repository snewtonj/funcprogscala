...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def timesAcc(chars: List[Char], 
      xs: List[(Char, Int)]): List[(Char, Int)] = {
      ...
      chars match {
        case List() => xs
      	case head :: tail  => timesAcc(tail, increment(head, xs))
      }	
    } 
    timesAcc(chars, List())
  }
   ...
   */
  def until(singleNode: List[CodeTree] => Boolean, combinedTree: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    trees match {
      case t if singleNode(t) => t
      case head :: tail => combinedTree(List(head, tail.head))
    }
  ...
  def createCodeTree(chars: List[Char]): CodeTree = 
    makeCodeTree(Leaf(chars.head, 1), createCodeTree(chars.tail))
   ...
   */
  def codeBits(table: CodeTable)(char: Char): List[Bit] = List(1)
   ...
   */
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = List(1)
}
